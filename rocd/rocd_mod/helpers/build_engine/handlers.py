...
from rocd_mod.helpers.build_engine.copy_step import do_add, do_copy
from rocd_mod.helpers.build_engine.errors import BuildError
from rocd_mod.helpers.build_engine.constants import PREDEFINED_ARGS
from rocd_mod.helpers.build_engine.parsing import (
    parse_kv_list, split_arg, to_argv,
)
from rocd_mod.helpers.build_engine.run_step import do_run
from rocd_mod.helpers.docker import layer_cache_path
from rocd_mod.helpers.layer_diff import write_files_layer
...
def do_cmd(engine, instr):
    """CMD [argv]/CMD command: default argv for `rocd run`."""
    cfg = engine.current.image_config.setdefault("config", {})
...
def do_healthcheck(engine, instr):
    ...
    # are accepted but not enforced under rocd.
