# Bash completion for rocd and pd
#
# Install system-wide:
#   cp rocd.bash /usr/share/bash-completion/completions/rocd
# Install for current user:
#   mkdir -p ~/.local/share/bash-completion/completions
#   cp rocd.bash ~/.local/share/bash-completion/completions/rocd

_rocd_mod_get_containers() {
    local dir
    if _rocd_mod_is_termux; then
        dir="${TERMUX__PREFIX:-/data/data/com.termux/files/usr}/var/lib/rocd/containers"
    else
        dir="${XDG_DATA_HOME:-${HOME}/.local/share}/rocd/containers"
    fi
    if [[ -d "${dir}" ]]; then
        local d
        for d in "${dir}"/*/; do
    ...
    case "${command}" in
        ...
        remove)
            if [[ "${cur}" == -* ]]; then
                ...
                COMPREPLY=($(compgen -W "$(_rocd_mod_get_containers)" -- "${cur}"))
            fi
            ;;
        ...
        copy)
            if [[ "${cur}" == -* ]]; then
                ...
                if [[ "${cur}" == *:* ]]; then
                    ...
                    local containers
                    containers=$(_rocd_mod_get_containers)
...
complete -F _rocd_mod rocd
complete -F _rocd_mod pd
