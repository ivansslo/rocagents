...
from rocd_mod.message import crit_error
from rocd_mod.constants import TERMUX_PREFIX
from rocd_mod.paths import container_rootfs


def get_device_cpu_arch() -> str:
    """Return the host CPU arch in rocd's naming scheme.
...
def _elf_arch(path: str) -> str:
    """Return the rocd arch name for an ELF binary, or '' on failure."""
    try:
...
# Docker platform strings and alternative names → rocd arch.
...
def normalize_arch(arch: str):
    """Return a canonical rocd arch name, or None if unrecognised.
...
# Machine string reported by `uname -m` for each rocd arch.
