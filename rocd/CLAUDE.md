...
`rocd` is a pure-Python utility for managing rootless,
...
https://pypi.org/project/rocd/. `pyproject.toml` is the version
source of truth; `PROGRAM_VERSION` reads it via `importlib.metadata`,
falling back to `"rolling"`. The shim `rocd.py` and console
scripts `rocd` / `pd` resolve to `rocd_mod.cli:main`.
Bash/Zsh/Fish completions ship under `rocd_mod/completions/`.
...
|---|---|---|
| `RUNTIME_DIR` | `$TERMUX_PREFIX/var/lib/rocd` | `$XDG_DATA_HOME/rocd` |
| `BASE_CACHE_DIR` | `$RUNTIME_DIR/cache` | `$XDG_CACHE_HOME/rocd` |
...
`su - other` doesn't drop the rocd-set vars: POSIX case-guard
