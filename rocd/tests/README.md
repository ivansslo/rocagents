...
End-to-end and unit tests for `rocd`. The suite is **offline and
deterministic** by default: no network, no real `proot` execution, and no
access to a real rocd installation.
...
`rocd_mod.constants` computes all runtime paths and `IS_TERMUX` **once at
...
the Termux/auth environment *before* importing any rocd_mod module, then
