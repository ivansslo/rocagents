# Shared pytest configuration for the rocd test suite.
#
# CRITICAL ORDERING: rocd_mod.constants computes RUNTIME_DIR /
...
# rocd_mod module is imported anywhere in the process. pytest imports the
...
# ---------------------------------------------------------------------------
# Sandbox the environment BEFORE importing rocd_mod.
# ---------------------------------------------------------------------------
...
from rocd_mod import constants, locking, message  # noqa: E402

import _builders  # noqa: E402


# ---------------------------------------------------------------------------
# Guard rails: never let the suite touch a real rocd installation.
# ---------------------------------------------------------------------------
...
def _assert_sandboxed():
    for path in (constants.RUNTIME_DIR, constants.BASE_CACHE_DIR):
        if not os.path.abspath(path).startswith(_SANDBOX):
            raise RuntimeError(
                f"rocd path {path!r} is not inside the test sandbox "
...
@pytest.fixture
def paths():
    """Expose the live (sandboxed) rocd path constants."""
    return constants


@pytest.fixture
def sandbox_tmp(tmp_path):
    """A scratch directory outside the rocd runtime tree."""
    return tmp_path
